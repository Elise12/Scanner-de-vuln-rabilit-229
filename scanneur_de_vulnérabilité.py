...
#                                  /
#
# *********************Bienvenue dans le scan rapide*********************************************
# Auteur  : Fidegnon Mahougnon Elisé                                                              ]
# WhatsApp :https://wa.me/+22965424731                                                          ]
# YouTube https://youtube.com/channel/UCvv_Gx64B0GMX-tHrQUyZmQ                           ]
# Outils   :  scanneur rapide                                                                         ]
# Usage	 : ./rapidscan.py example.com (or) python rapidsan.py example.com           .         ]
# Description: Ce scanner automatise le processus d'analyse de sécurité en utilisant un         ]
#              multitude d'outils de sécurité Linux disponibles et quelques scripts personnalisés. ]
# ***************************[+22965424731]*******************************************************
...
def helper():
        ...
        print "--------"
        print "\t["+proc_high+"]: le processus de scan prendra assez de temps (non predictable)."
        print "\t["+proc_med+"]: le processus de scan durera environ 10 minutes."
        print "\t["+proc_low+"]: le processus de scan prendra une minute ou deux."
        print bcolors.OKBLUE+"Vulnerability Information:"+bcolors.ENDC
        print "--------------------------"
        print "\t"+vul_info('c')+": Nécessite une attention immédiate car cela peut entraîner un compromis ou une indisponibilité du service."
        print "\t"+vul_info('h')+"    : Peut ne pas conduire à un compromis immédiat, mais il y a de fortes chances de probabilité."
        print "\t"+vul_info('m')+"  : L'attaquant peut corréler plusieurs vulnérabilités de ce type pour lancer une attaque sophistiquée."
        print "\t"+vul_info('l')+"     : Pas un problème grave, mais il est recommandé d'assister à la découverte."
        print "\t"+vul_info('i')+"    : Non classé comme une vulnérabilité, simplement une alerte informative utile à prendre en compte.\n"
...
def logo():
	print bcolors.WARNING
        print("""\
                                ...
                                / ( (//)/(/__)( (//)
                                     /
                     """+bcolors.ENDC+"""(Le multi - outils de scannage du siteweb)
                            """)
        print bcolors.ENDC

# Initialisation de la classe de chargeur/spinner inactif
class Spinner:
    busy = False
    delay = 0.05
...
# Fin de la classe chargeur/spinner

# Instanciation de la classe spinner/loader
spinner = Spinner()



# le scanneur pourra être utilisé et rotation du nom du fichier (default: désactivé (1))
tool_names = [
...
# Résultats de l'outil (Fin)



# Statut d'outils(Response Data + Code du résultat(if status check fails and you still got to push it + Legends + Approx Time + Tool Identification + Bad Responses)
tool_status = [
...
# Lecture aléatoire de l'ordre (début)
...
# Lecture aléatoire de l'ordre (fin)

# Pointeur de la tête d'outil : (peut être augmenté mais certains outils seront ignorés)
tool = 0
...
if args_namespace.help or (not args_namespace.update \
    ...
    logo()
    print "Mise à jour de scanneur rapide.... Veuillez patienter.\n"
    spinner.start()
    # Checking internet connectivity first...
    rs_internet_availability = check_internet()
    if rs_internet_availability == 0:
        print "\t"+ bcolors.BG_ERR_TXT + "Il semble y avoir un problème de connexion à Internet.  Veuillez réessayer ou plus tardr." +bcolors.ENDC
        spinner.stop()
        sys.exit(1)
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print "\t"+ bcolors.OKBLUE +"Vous êtes déjà prêt à avoir la dernière version de RapidScan." + bcolors.ENDC
    else:
        clear()
        print "\t"+ bcolors.OKGREEN +"Dernière version de apidScan mise à jour avec succès ." +bcolors.ENDC
    spinner.stop()
    sys.exit(1)
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print "\t"+bcolors.BG_ERR_TXT+"RapidScan a été interrompu brutalement..."+bcolors.ENDC
            sys.exit(1)
        ...
        if "not found" in val or tools_precheck[rs_avail_tools][arg1] in args_namespace.skip :
            ...
            print "\t"+bcolors.OKBLUE+tools_precheck[rs_avail_tools][arg1]+bcolors.ENDC+bcolors.OKGREEN+"... disponible."+bcolors.ENDC
        rs_avail_tools = rs_avail_tools + 1
        clear()
    unavail_tools_names = list(set(unavail_tools_names))
    if len(unavail_tools_names) == 0:
        print "\t"+bcolors.OKGREEN+"Tous les outils de scans sont disponibles.  Toutes les vérifications de vulnérabilité seront effectuées par RapidScan"+bcolors.ENDC
    else:
        print "\t"+bcolors.WARNING+"Certaines de ces outils "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. RapidScan will still perform the rest of the tests. Install these tools to fully utilize the functionality of RapidScan."+bcolors.ENDC
    print bcolors.BG_ENDL_TXT+"[ Vérification de la phase des outils d'analyse de sécurité disponibles... Terminé. ]"+bcolors.ENDC
    print "\n"
    print bcolors.BG_HEAD_TXT+"[ Phase d'analyse préliminaire lancée... Chargé "+str(tool_checks)+" vulnerability checks.  ]"+bcolors.ENDC
    #while (tool < 1):
    while(tool < len(tool_names)):
        ...
        if runTest == 1:
                ...
                clear()
                print "\t"+bcolors.WARNING + "Test ignorée. Exécution suivante. Press Ctrl+Z to Quit RapidScan." + bcolors.ENDC
                rs_skipped_checks = rs_skipped_checks + 1

        tool=tool+1

    print bcolors.BG_ENDL_TXT+"[ Phase d'analyse préliminaire terminée. ]"+bcolors.ENDC
    print "\n"

    #################### Rapport et documentation de phase ###########################
    print bcolors.BG_HEAD_TXT+"[ Phase de génération de rapport lancée. ]"+bcolors.ENDC
    if len(rs_vul_list)==0:
        print "\t"+bcolors.OKGREEN+"Pas de vulnérabilité détectées."+bcolors.ENDC
    else:
        with open("RS-Vulnerability-Report", "a") as report:
            ...
            print "\tRapport de vulnérabilité complet pour "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+"`RS-Vulnerability-Report`"+bcolors.ENDC+" est disponible sous le même répertoire que RapiScan réside."

        report.close()
    # Écriture de tous les fichiers d'analyse générés dans RS-Debug-ScanLog à des fins de débogage.
    ...
    print "\tNombre total de vulnérabilités vérifiées        : "+bcolors.BOLD+bcolors.OKGREEN+str(len(tool_names))+bcolors.ENDC
    print "\tNombre total de vulnérabilités vérifiées ignorré : "+bcolors.BOLD+bcolors.WARNING+str(rs_skipped_checks)+bcolors.ENDC
    print "\tNombre total de vulnérabilités détectées    : "+bcolors.BOLD+bcolors.BADFAIL+str(len(rs_vul_list))+bcolors.ENDC
    print "\tL'heure totale épuisée lors du scan             : "+bcolors.BOLD+bcolors.OKBLUE+display_time(int(rs_total_elapsed))+bcolors.ENDC
    print "\n"
    print "\tÀ des fins de débogage, vous pouvez afficher la sortie complète générée par tous les outils nommés "+bcolors.OKBLUE+"`RS-Debug-ScanLog`"+bcolors.ENDC+" under the same directory."
    print bcolors.BG_ENDL_TXT+"[ Phase de génération de rapport terminée. ]"+bcolors.ENDC

    os.system('setterm -cursor on')
    os.system('rm te* > /dev/null 2>&1') # Nettoyage fichiers d'analyse précédents
